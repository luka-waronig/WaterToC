"""
Render 3D surface plots of the *cooperation* tail medians over (ρ_w, Kmax),
faceted by θ. For each θ:
1) Compute per-run tail medians of Coop_Fraction (last TAIL_FRAC of steps),
2) Take the median across runs per (θ, Kmax, ρ_w),
3) Interpolate onto a regular grid and plot a colored surface with a shared scale.
To be able to run, first run sweep_run.py.
"""
...
OUTDIR   = "results"
FIG_NAME = "coop_surfaces_by_theta.png"
...
def tail_median_per_run(g: pd.DataFrame, frac: float) -> float:
    """
    Tail median of Coop_Fraction for a single run.
    Args:
        g: DataFrame for one (theta, Kmax, rho_w, seed) with 'Step' and 'Coop_Fraction'.
        frac: Fraction (0–1] of *final* steps to include in the tail window.
    Returns:
        Median Coop_Fraction over the tail window.
    """
    g = g.sort_values("Step")
    n = len(g)
    take = max(1, int(np.ceil(frac * n)))
    return g["Coop_Fraction"].iloc[-take:].median()

per_run = (
    df.groupby(["theta","max_water_capacity","water_cell_density","seed"], as_index=False)
      .apply(lambda g: pd.Series({"Coop_tail_median_run": tail_median_per_run(g, TAIL_FRAC)}))
)

agg = (
    per_run.groupby(["theta","max_water_capacity","water_cell_density"], as_index=False)
           .agg(Coop_tail_median=("Coop_tail_median_run","median"))
).sort_values(["theta","max_water_capacity","water_cell_density"]).reset_index(drop=True)

thetas = list(np.sort(agg["theta"].unique()))

vmin = float(agg["Coop_tail_median"].min())
vmax = float(agg["Coop_tail_median"].max())
norm = colors.Normalize(vmin=vmin, vmax=vmax)

# Use inferno to match env-plot look, or keep viridis for contrast
cmap = cm.viridis
...
for idx, th in enumerate(thetas):
    ...
    vals = sub["Coop_tail_median"].values
    ...
    ax.set_zlabel("Cooperation", labelpad=10)
...
if last_surf is not None:
    ...
    cb.set_label("Cooperation", rotation=90, labelpad=14, va='center')
